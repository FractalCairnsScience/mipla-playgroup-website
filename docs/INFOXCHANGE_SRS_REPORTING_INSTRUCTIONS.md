# Reporting on Infoxchange SRS: Simple Outreach-Worker Instructions

**Prepared for:** Mura Moegi Kaziw Sagulau Playgroup outreach workers  
**Prepared by:** Manus AI  
**Source:** Infoxchange SRS User Guide — July 2011, version 1.4, supplied by the user.

> These instructions summarise the supplied 2011 guide. The current Infoxchange SRS interface, organisational procedures and user permissions may differ. Follow Mura’s current Infoxchange SRS procedures and ask the site administrator if your screen, report options or workgroup names are different.
...
| Read plan for reporting | Local PDF or printed copy | Enter only the authorised information into Infoxchange SRS. |
| Finish reporting | Mura-approved local file store / Infoxchange SRS | Close Infoxchange SRS, lock the device and delete temporary copies where appropriate. |

## 2. Before you start

Open the Infoxchange SRS website address assigned to your organisation. Enter your organisation-issued username and password, then select **Submit**. The supplied guide notes that usernames are case-sensitive, that an account can be locked after five unsuccessful attempts, and that an account may also be locked after more than six weeks without access.[1]
...
## 3. Generate a standard Infoxchange SRS report
...
```mermaid
flowchart TD
    A[Log in with your own Infoxchange SRS account] --> B[Confirm correct Workgroup]
    B --> C[Reports tab]
    C --> D[Choose report type and information to include]
    D --> E[Choose reporting period or exact dates]
    E --> F[Check selections against Mura's approved reporting requirement]
    F --> G[Generate Report]
    G --> H[Record report ID]
    H --> I[Results tab: open and review report]
    I --> J[Store or share only through Mura-approved channels]
```
...
Before you save, export or share an Infoxchange SRS output, pause and check that you have selected the right Workgroup, date range and approved report type. Confirm that personal names appear only when necessary and authorised. Read the Results output for obvious omissions, duplicate records or unexpected information. If you think a report has been generated with the wrong settings, do not distribute it; contact the responsible Mura supervisor or Infoxchange SRS site administrator for the correct process.

## Reference

[1] *Infoxchange SRS User Guide*, July 2011, version 1.4, supplied by the user. Relevant sections: 1.0–2.1, 6.0–6.5 and 8.0.
